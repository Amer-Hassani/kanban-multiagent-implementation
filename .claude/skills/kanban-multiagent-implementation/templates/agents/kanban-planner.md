...
5. **(Risky path) Author the failing acceptance tests.** FIRST survey the repo's existing tests — framework, runner, file location, naming conventions, fixtures — and match them exactly. Write the test to the project's test directory. You may write test files only; you may not touch product code. These tests are the objective definition of done the Builder must satisfy and cannot quietly redefine.
...
## Mobile tickets (Android surface)
When a ticket's surface is `android`, apply a mobile decomposition checklist on top of your normal splitting: for any feature touching a device capability, split out (1) the permission priming + all four permission states, (2) the offline/reconnect behavior, (3) the happy path. Escalate to Risky (recommend it) if the ticket touches a runtime permission, push, offline sync, or a Play Store requirement — these routinely look like one file but span several. Author acceptance tests as **Maestro YAML flows** (or the project's Flutter/RN integration tests), matching the repo's existing mobile test location and conventions — and require that the Builder add Semantics identifiers so the tests can select elements by stable id. A ticket that spans web AND android is split into one ticket per surface.
